...
class OpenCVConan(ConanFile):
    ...
    options.update({_name: [True, False] for _name, _ in OPENCV_MAIN_MODULES_OPTIONS})
    options.update({_name: [True, False] for _name, _ in OPENCV_EXTRA_MODULES_OPTIONS})
    ...
    default_options.update(
        {_name: select for _name, select in OPENCV_MAIN_MODULES_OPTIONS}
    )
    default_options.update(
        {_name: select for _name, select in OPENCV_EXTRA_MODULES_OPTIONS}
    )
    ...
    def config_options(self):
        ...
        if not self._has_with_ffmpeg_option:
    ...
    def _opencv_modules(self):
        ...
        opencv_modules = {
            ...
            "core": {
                ...
                "frameworks": [
                    (
                        ...
                        and self.options.get_safe("with_opencl"),
                        ["OpenCL"],
                    ),
                ],
            },
            "dnn": {
                ...
                "requires": ["opencv_core", "opencv_imgproc", "protobuf::protobuf"]
                + vulkan()
                + ipp(),
            },
            "features2d": {
            ...
            "imgcodecs": {
                ...
                "mandatory_options": ["imgproc"],
                "requires": ["opencv_imgproc", "zlib::zlib"]
                + imageformats_deps()
                + ipp(),
                "frameworks": [
            ...
            "objdetect": {
                ...
                "requires": ["opencv_core", "opencv_calib3d", "opencv_imgproc"]
                + quirc()
                + ipp(),
            },
            "photo": {
            ...
            "videoio": {
                ...
                "system_libs": [
                    (
                        ...
                        and int(str(self.settings.os.api_level)) > 20,
                        ["mediandk"],
                    ),
                ],
                "frameworks": [
            ...
            "cudaimgproc": {
                ...
                "requires": ["opencv_imgproc", "opencv_cudev"]
                + opencv_cudaarithm()
                + opencv_cudafilters()
                + ipp(),
            },
            "cudalegacy": {
                ...
                "mandatory_options": ["with_cuda", "video"],
                "requires": ["opencv_core", "opencv_video"]
                + opencv_calib3d()
                + opencv_imgproc()
                + opencv_objdetect()
                + opencv_cudaarithm()
            ...
            "dpm": {
                ...
                "requires": ["opencv_core", "opencv_imgproc", "opencv_objdetect"]
                + ipp(),
            },
            "face": {
            ...
            "superres": {
                ...
                "requires": ["opencv_imgproc", "opencv_video", "opencv_optflow"]
                + opencv_videoio()
                + ipp()
                + opencv_cudaarithm()
                + opencv_cudafilters()
            ...
            "tracking": {
                ...
                "requires": ["opencv_core", "opencv_imgproc", "opencv_video"]
                + opencv_dnn()
                + ipp(),
            },
            "videostab": {
            ...
            "ippiw": {
                "is_built": self.options.with_ipp == "opencv-icv"
            ...
            "correspondence": {
                ...
                "is_part_of_world": False,
                "no_option": True,
                "requires": ["opencv_imgcodecs", "multiview", "glog::glog"]
                + eigen()
                + ipp(),
            },
            "multiview": {
        ...
        if Version(self.version) >= "4.4.0":
            opencv_modules["intensity_transform"].setdefault(
                "mandatory_options", []
            ).append("imgproc")
            opencv_modules["intensity_transform"].setdefault("requires", []).append(
                "opencv_imgproc"
            )
        if Version(self.version) < "4.3.0":
            opencv_modules["stereo"].setdefault("mandatory_options", []).extend(
                ["calib3d", "video"]
            )
            opencv_modules["stereo"].setdefault("requires", []).extend(
                ["opencv_calib3d", "opencv_video"]
            )
    ...
    def _get_mandatory_disabled_options(self, opencv_modules):
        ...
        for base_option in base_options:
            for mandatory_option in opencv_modules.get(base_option, {}).get(
                ...
                if not self.options.get_safe(mandatory_option):
                    direct_options_to_enable.setdefault(mandatory_option, set()).add(
                        base_option
                    )
        ...
        def collect_transitive_options(base_option, option):
            for mandatory_option in opencv_modules.get(option, {}).get(
                "mandatory_options", []
            ):
                if not self.options.get_safe(mandatory_option):
                    ...
                    if base_option not in direct_options_to_enable.get(
                        mandatory_option, set()
                    ):
    ...
    def _solve_internal_dependency_graph(self, opencv_modules):
        ...
        if all_options_to_enable:
            ...
            for option_to_enable in all_options_to_enable:
                ...
                transitive = ", ".join(
                    transitive_options_to_enable.get(option_to_enable, [])
                )
                if transitive:
                    direct_and_transitive.append(
                        f"transitive dependency of {transitive}"
                    )
                message += (
                    f"  - {option_to_enable}: {' / '.join(direct_and_transitive)}\n"
                )
    ...
    def configure(self):
        ...
        if self.options.contrib != "deprecated":
            ...
            if self.options.contrib:
                ...
                filtered_options = [module for module, _ in OPENCV_MAIN_MODULES_OPTIONS]
        ...
        if self.options.contrib_freetype != "deprecated":
            self.output.warning(
                "contrib_freetype option is deprecated, use freetype option instead"
            )
        ...
        if self.options.contrib_sfm != "deprecated":
            self.output.warning(
                "contrib_sfm option is deprecated, use sfm option instead"
            )
        ...
        if self.options.with_ade != "deprecated":
            self.output.warning(
                "with_ade option is deprecated, use gapi option instead"
            )
    ...
    def _check_mandatory_options(self, opencv_modules):
        ...
        all_disabled_mandatory_options.update(
            transitive_disabled_mandatory_options.keys()
        )
        if all_disabled_mandatory_options:
            ...
            for disabled_option in all_disabled_mandatory_options:
                direct_and_transitive = []
                direct = ", ".join(
                    direct_disabled_mandatory_options.get(disabled_option, [])
                )
                if direct:
                    direct_and_transitive.append(f"direct dependency of {direct}")
                transitive = ", ".join(
                ...
                message += (
                    f"  - {disabled_option}: {' / '.join(direct_and_transitive)}\n"
                )
    ...
    def validate(self):
        ...
        if self.options.get_safe("dnn_cuda") and (
            ...
            raise ConanInvalidConfiguration(
                "with_cublas and with_cudnn must be enabled for dnn_cuda"
            )
        if self.options.with_ipp == "opencv-icv" and not (
            self.settings.arch in ["x86", "x86_64"]
            and self.settings.os in ["Linux", "Macos", "Windows"]
        ):
            raise ConanInvalidConfiguration(
    ...
    def _patch_sources(self):
        ...
        replace_in_file(
            self,
            os.path.join(
                self.source_folder, "modules", "videoio", "cmake", "detect_ffmpeg.cmake"
            ),
            "FFMPEG_FOUND",
            "ffmpeg_FOUND",
        )
        ...
        if self.options.dnn:
            find_protobuf = os.path.join(
                self.source_folder, "cmake", "OpenCVFindProtobuf.cmake"
            )
    ...
    def generate(self):
        ...
        if self.options.get_safe("with_ffmpeg"):
            ...
            for component in [
                ...
                if component == "avutil" or self.dependencies[
                    "ffmpeg"
                ].options.get_safe(component):
                    ffmpeg_libraries.append(f"ffmpeg::{component}")
                    ffmpeg_component_version = (
                        self.dependencies["ffmpeg"]
                        .cpp_info.components[component]
                        .get_property("component_version")
                    )
                    tc.variables[
                        f"FFMPEG_lib{component}_VERSION"
                    ] = ffmpeg_component_version
        ...
        tc.variables["WITH_IMGCODEC_HDR"] = self.options.get_safe(
            "with_imgcodec_hdr", False
        )
        tc.variables["WITH_IMGCODEC_PFM"] = self.options.get_safe(
            "with_imgcodec_pfm", False
        )
        tc.variables["WITH_IMGCODEC_PXM"] = self.options.get_safe(
            "with_imgcodec_pxm", False
        )
        tc.variables["WITH_IMGCODEC_SUNRASTER"] = self.options.get_safe(
            "with_imgcodec_sunraster", False
        )
        ...
        if self.options.get_safe("with_vulkan"):
            tc.variables["VULKAN_INCLUDE_DIRS"] = os.path.join(
                self.dependencies["vulkan-headers"].package_folder, "include"
            ).replace("\\", "/")
        ...
        if self._has_with_jpeg2000_option:
            tc.variables["WITH_JASPER"] = (
                self.options.get_safe("with_jpeg2000") == "jasper"
            )
        ...
        for module, select in OPENCV_MAIN_MODULES_OPTIONS:
            tc.variables[f"BUILD_opencv_{module}"] = self.options.get_safe(
                module, False
            )
        ...
        if self.options.objdetect:
            tc.variables[
                "HAVE_QUIRC"
            ] = self.options.with_quirc  # force usage of quirc requirement
        ...
        for module, _ in OPENCV_EXTRA_MODULES_OPTIONS:
            tc.variables[f"BUILD_opencv_{module}"] = self.options.get_safe(
                module, False
            )
    ...
    def package_info(self):
        ...
        def get_libs(module):
            if module == "ippiw":
                ...
                if module in ["core", "world"] and not self.options.shared:
                    lib_exclude_filter = (
                        "(opencv_|ippi|correspondence|multiview|numeric).*"
                    )
                    libs += list(
                        filter(
                            lambda x: not re.match(lib_exclude_filter, x),
                            collect_libs(self),
                        )
                    )
                return libs
        ...
        def add_components(modules):
            if self.options.world:
                ...
                if self.settings.os != "Windows":
                    self.cpp_info.components["opencv_world"].includedirs.append(
            ...
            for module, values in modules.items():
                ...
                if self.options.world and values.get("is_part_of_world", True):
                    self.cpp_info.components[conan_component].requires = [
                        "opencv_world"
                    ]
                    ...
                    self.cpp_info.components[
                        conan_component
                    ].system_libs = module_system_libs
                    self.cpp_info.components[
                        conan_component
                    ].frameworks = module_frameworks
            ...
            if self.options.world:
                ...
                self.cpp_info.components["opencv_world"].system_libs = list(
                    world_system_libs
                )
                self.cpp_info.components["opencv_world"].frameworks = list(
                    world_frameworks
                )

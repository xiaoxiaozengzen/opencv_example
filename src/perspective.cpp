 ...
 *    在计算机视觉中又叫单应性变换(homography)，是指将图像中的一个平面映射到另一个平面的一种变换。
 *    透视变换可以用一个3x3的矩阵来表示，该矩阵可以将图像中的点从一个平面映射到另一个平面。
 *    透视变换的数学表示为：
 *    [x']   [h11 h12 h13]   [x]
 *    [y'] = [h21 h22 h23] * [y]
 *    [w']   [h31 h32 h33]   [1]
 *    其中，(x, y)是原图像中的点，(x', y')是变换后的点，(h11, h12, h13, h21, h22, h23, h31, h32, h33)是透视变换矩阵的元素，w'是归一化因子。
 *    透视变换可以实现图像的旋转、缩放、平移、倾斜等操作，常用于图像的校正、拼接、增强等任务。
 *    一般H33=1
 * 单应性变换(homography)在以下场景严格成立：
 * 1. 当场景是平面时，单应性变换可以精确地描述图像之间的关系。
 * 2. 映射完之后直线仍然是直线。

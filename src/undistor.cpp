...
void fisheye_undistor_sequnce(const CameraParams& params) {
    ...
    // 使用remap函数进行去畸变
    cv::Mat input_image_bgr = params.image;
    cv::Mat undistorted_image_bgr;
    cv::remap(input_image_bgr, undistorted_image_bgr, map1, map2, cv::INTER_LINEAR);
    cv::imwrite(params.output_undistorted_image_path, undistorted_image_bgr);    
}
 ...
 */
void resize_with_intrinsics() {
    // 原始的FW相机内参矩阵，去畸变参数，图像尺寸
    cv::Mat k = (cv::Mat_<double>(3, 3) << 1906.6, 0, 1923.26
                                            , 0, 1906.18, 1022.45
                                            , 0, 0, 1);
    std::vector<double> dist_coeffs = {-0.0299548, -0.00364585, -0.00155829, 0.00104736};
    ...
    // 调整因子
    double scale_w = 0.6; // 水平缩放因子
    double scale_h = 0.6; // 垂直缩放因子
    cv::Size resized_image_size(static_cast<int>(image_size.width * scale_w), static_cast<int>(image_size.height * scale_h));

    // 读取输入图像并
    std::string input_root_path = "/mnt/workspace/cgz_workspace/Exercise/opencv_example/image/";
    std::string output_root_path = "/mnt/workspace/cgz_workspace/Exercise/opencv_example/output/";
    std::string input_image_path = input_root_path + "frontwide_3840_2048_420p.yuv";
    std::ifstream input_image_stream(input_image_path, std::ios::binary | std::ios::ate);
    if(!input_image_stream.is_open()) {
        std::cerr << "Could not open input image file: " << input_image_path << std::endl;
        return;
    ...
    cv::Mat input_image_bgr;
    cv::cvtColor(input_image, input_image_bgr, cv::COLOR_YUV2BGR_I420); // YUV420格式转换为BGR格式
    std::string output_input_image_path = output_root_path + "resize_test_original_bgr.jpg";
    cv::imwrite(output_input_image_path, input_image_bgr);

    // 对图像进行resize
    cv::Mat resized_image_bgr;
    cv::resize(input_image_bgr, resized_image_bgr, resized_image_size, 0, 0, cv::INTER_LINEAR);
    std::string output_resized_image_path = output_root_path + "resize_test_resized_bgr.jpg";
    cv::imwrite(output_resized_image_path, resized_image_bgr);

    // resize后的图片进行去畸变
    CameraParams params;
    params.K = k;
    params.K.at<double>(0, 0) *= scale_w; // fx缩放
    params.K.at<double>(1, 1) *= scale_h; // fy缩放
    params.K.at<double>(0, 2) *= scale_w; // cx缩放
    params.K.at<double>(1, 2) *= scale_h; // cy缩放
    params.dist_coeffs = dist_coeffs;
    params.ori_image_size = resized_image_size;
    params.undistorted_image_size = resized_image_size;
    params.balance = 1.0;
    params.image = resized_image_bgr;
    params.output_undistorted_image_path = output_root_path + "resize_test_resized_undistorted_bgr.jpg";
...
int main() {
    ...
    std::cout << "======================== resize_with_intrinsics ========================" << std::endl;

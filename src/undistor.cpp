...
void fl_undistor_sequnce() {
    ...
    cv::Mat input_image_bgr;
    cv::cvtColor(input_image, input_image_bgr, cv::COLOR_YUV2BGR_I420); // YUV420格式转换为BGR格式
    cv::imwrite("/mnt/workspace/cgz_workspace/Exercise/opencv_example/output/frontlong_3840_2048_420p_bgr.jpg", input_image_bgr);
    cv::Mat undistorted_image_bgr;
    cv::remap(input_image_bgr, undistorted_image_bgr, map1, map2, cv::INTER_LINEAR);
    std::string output_image_path = "/mnt/workspace/cgz_workspace/Exercise/opencv_example/output/frontlong_3840_2048_420p.jpg";
    cv::imwrite(output_image_path, undistorted_image_bgr);
}
...
int main() {
    ...
    fw_undistor_sequnce();
    std::cout << "======================== fl_undistor_sequnce ========================" << std::endl;
    fl_undistor_sequnce();
